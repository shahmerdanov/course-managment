...
public class StudentsServiceImpl implements StudentsService {
    ...
    public StudentResponse getStudentByNameAndSurname(String name, String surname) {
        List<StudentDto> studentss = studentsRepository.findByNameAndSurname(name, surname)
                .stream()
                .map(students -> convertToDto(students))
                .collect(Collectors.toList());
        return makeStudentResponse(studentss);
    }
    ...
    public void updateStudents(StudentDto studentsDTO, long id) {
        ...
        students.setAge(studentsDTO.getAge());
        students.setGroupName(students.getGroupName());
        studentsRepository.save(students);
    }
    ...
    public void updateSomeStudents(StudentDto studentsDTO, long id) {
        ...
        if (studentsDTO.getName()!=null)
        students.setName(studentsDTO.getName());

        if (studentsDTO.getSurname()!=null)
        students.setSurname(studentsDTO.getSurname());

        if (studentsDTO.getAge()>0)
        students.setAge(studentsDTO.getAge());

        if (studentsDTO.getGroupName()!=null)
        students.setGroupName(students.getGroupName());
